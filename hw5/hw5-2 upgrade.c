...
int main(){
    int my_bgo[5][5], com_bgo[5][5], make_num[25], com_bgo_imp[5][5] = {2};
    int sel_num, my_open_num[25] = {0}, com_open_num[25] = {0};
    int round = 1, k, y_dup, z_dup;
    int my_bgo_cnt = 0, com_bgo_cnt = 0, dup_cnt = 0, imp_cnt = 0;
    ...
    while(my_bgo_cnt < 5 && com_bgo_cnt < 5){
        ...
        printf("\n\n\n");
        if(round % 2 == 0){
            if(imp_cnt > 8) sel_num = rand() % 25 + 1;
            else if(imp_cnt > 1){
                if(com_bgo_imp[i][j] == 3)
            } 
            y_dup = 0;
            for(int x = 0; x < 25; x++){
